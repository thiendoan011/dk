...
export abstract class ComponentBase {
    ...
    moneyTextToNumberFRound2(moneyText) {
        ...
        return this.round(parseFloat(moneyText.replace(/,/g, '')));
    }

    moneyTextToNumberFRound3(moneyText) {
        if (moneyText == '' || moneyText == null || moneyText === undefined) {
            return undefined;
        }

        return this.round3(parseFloat(moneyText.replace(/,/g, '')));
    }

    moneyTextToNumberFRound4(moneyText) {
        if (moneyText == '' || moneyText == null || moneyText === undefined) {
            return undefined;
        }

        return this.round4(parseFloat(moneyText.replace(/,/g, '')));
    }

    moneyTextToNumberFRound5(moneyText) {
        if (moneyText == '' || moneyText == null || moneyText === undefined) {
            return undefined;
        }

        return this.round5(parseFloat(moneyText.replace(/,/g, '')));
    }
    ...
    isNullOrEmptyNumber(value: number){
    ...
    isNullOrEmptyOrDraftOrReject(value: string){
        if(value === null || value === undefined || value === '' || value == 'E' || value == 'R'){
            return true;
        }
        else{
            return false;
    ...
    // Dùng cho việc click xem lịch sử(icon arrow chuyển từ right --> down và ngược lại): 
    // Sử dụng bằng cách khai báo trong ngOnInit
    ...
    // Chuyển số thập phần --> phân số
    gcd(a,b){ 
        return b ? this.gcd(b, a%b) : a; 
    }; 
    
    reduce(number,denomin){ 
    var gcd = this.gcd(number,denomin); 
