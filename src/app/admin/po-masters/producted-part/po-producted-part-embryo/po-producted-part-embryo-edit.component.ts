...
import { CM_DEPARTMENT_ENTITY, UltilityServiceProxy, PO_ENTITY, PoProductedPartDetailServiceProxy, PO_PRODUCTED_PART_ENTITY, PO_GROUP_PRODUCT_ENTITY } from '@shared/service-proxies/service-proxies';
...
import { PPPEPartDetailEditTableComponent } from './pppe-part-detail-edittable.component';
...
import { POPOProductDetailEdittableComponent } from '../po-po-product-details/po-po-product-detail-edittable.component';

@Component({
    templateUrl: './po-producted-part-embryo-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class PoProductedPartEmbryoEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionRejectExt<PO_ENTITY> {
    ...
    ngOnInit(): void {
        ...
        switch (this.editPageState) {
            case EditPageState.add:
                ...
                this.appToolbar.setRole('PoProductedPartDetail', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.disablePoEdit = true;
                this.appToolbar.setRole('PoProductedPartDetail', false, false, false, false, false, false, true, false);
    ...
    goBack() {
        this.navigatePassParam('/app/admin/po-producted-part-embryo', null, { filterInput: JSON.stringify(this.filterInput) });
    }
        ...
        this.poProductedPartDetailService
			...
			.subscribe((res) => {
				if (res['Result'] != '0') {
					...
					this.updateSuccess();
                    // CM_ATTACH_FILE
					this.updateFile(this.inputModel, 'po-producted-part-embryo', undefined, this.inputModel.pO_PRODUCTED_PART_ID);
                    this.getPoProductedPartDetail();
                    this.updateView();
				}
			});
    ...
    onStartProductedPart(){
        abp.ui.setBusy()
        this.poProductedPartDetailService
			.pO_PRODUCTED_PART_DETAIL_Start(this.inputModel.pO_ID, "CD1", "Công đoạn phôi")
...
//#region "EditTable"
    ...
    // Chi tiết sản phẩm PO
    @ViewChild('poPOProductDetailEdittable') poPOProductDetailEdittable: POPOProductDetailEdittableComponent;

//#endregion "EditTable"

    onSelectGroupProduct(event: PO_GROUP_PRODUCT_ENTITY){
        /*
        this.poProductedPartDetailService.pO_PRODUCTED_PART_GROUP_PRODUCT_Byid(this.inputModel.pO_ID, 'CD1', event.grouP_PRODUCT_ID).subscribe(res => {

            this.pppePartDetailEditTable.inputModel.pO_ID = event.pO_ID;    
            this.pppePartDetailEditTable.inputModel.grouP_PRODUCT_ID = event.grouP_PRODUCT_ID;
            this.pppePartDetailEditTable.inputModel.grouP_PRODUCT_CODE = event.grouP_PRODUCT_CODE;
            this.pppePartDetailEditTable.inputModel.grouP_PRODUCT_NAME = event.grouP_PRODUCT_NAME;
            this.pppePartDetailEditTable.updateView();

            // Set danh sách tiến độ hệ hàng
            if (res.pO_GROUP_PRODUCT_PART_DETAILs && res.pO_GROUP_PRODUCT_PART_DETAILs.length > 0) {
                this.pppePartDetailEditTable.editTable.setList(res.pO_GROUP_PRODUCT_PART_DETAILs);
                this.pppePartDetailEditTable.refreshTable();
            }
            else{
                this.pppePartDetailEditTable.editTable.setList([]);
                this.pppePartDetailEditTable.refreshTable();
            }
    ...
    onCompleteEditProductedPartGroupProduct(){
        this.updateView();
    }
