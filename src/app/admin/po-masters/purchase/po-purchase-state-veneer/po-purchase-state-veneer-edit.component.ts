...
@Component({
    templateUrl: './po-purchase-state-veneer-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class PoPurchaseStateVeneerEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionRejectExt<PO_ENTITY> {
    ...
    checkApproveOfCurrentUser() {
        this.poMasterService
            .cM_CHECK_ROLE_Edit(this.inputModel.pO_PRODUCTED_PART_VENEER_ID, 'PO_PURCHASE_STATE_VENEER', this.appSession.user.userName)
            .subscribe(res => {
                if (res['Result'] !== '0') {
    ...
    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                ...
                this.appToolbar.setRole('PoPurchaseStateVeneer', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('PoPurchaseStateVeneer', false, false, false, false, false, false, true, false);
    ...
    goBack() {
        this.navigatePassParam('/app/admin/po-purchase-state-veneer', null, { filterInput: JSON.stringify(this.filterInput) });
    }
    ...
    onUpdate(): void {
        ...
        this.poProductedPartVeneerService
			.pO_PURCHASE_STATE_VENEER_Upd(this.inputModel)
        ...
        this.poProductedPartVeneerService
			...
			.subscribe((res) => {
				if (res['Result'] != '0') {
                    ...
                    this.poProductedPartVeneerService
                        .pO_PURCHASE_STATE_VENEER_Appr(this.inputModel.pO_PRODUCTED_PART_VENEER_ID, this.appSession.user.userName)
                        .pipe( finalize(() => { this.saving = false; }) )
                        .subscribe((res) => {
                            if (res['Result'] != '0') {
                                this.showErrorMessage(res['ErrorDesc']);
    ...
    exportExcelPurchaseDT():void {
        ...
        reportInfo.pathName = '/PO_MASTER/rpt_PO_PURCHASE_STATE_VENEER_DT.xlsx';
    ...
    onChangeAddWarehouse(item: PO_PRODUCTED_PART_VENEER_SPECIFICATION_ENTITY){
        item.remaiN_NUMBER = item.quantitY_REQUEST - item.adD_WAREHOUSE_NUMBER;
        this.updateView();
    }
