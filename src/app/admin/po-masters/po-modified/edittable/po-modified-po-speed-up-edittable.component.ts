...
import { PO_MODIFIED_ENTITY, PO_ENTITY, PO_MODIFIED_PO_SPEED_UP_EDITTABLE } from "@shared/service-proxies/service-proxies";
import * as moment from "moment";

@Component({
    selector: 'po-modified-po-speed-up-edittable',
	templateUrl: './po-modified-po-speed-up-edittable.component.html',
	encapsulation: ViewEncapsulation.None,
	changeDetection: ChangeDetectionStrategy.OnPush,
	animations: [ appModuleAnimation() ]
})

export class POModifiedPOSpeedUpEdittableComponent extends ChangeDetectionComponent implements AfterViewInit {
//#region "Constructor"
    constructor(
        injector: Injector,
    ) {
...
//#endregion "Constructor"    
    ...
    @ViewChild('editTable') editTable: EditableTableComponent<PO_MODIFIED_PO_SPEED_UP_EDITTABLE>;
    ...
    showPopup(): void {
        ...
        //this.popupModal.filterInput.frmdate = moment();
		this.popupModal.show();
	}
	
	onSelectPopup(items: PO_ENTITY[]): void {
		items.forEach(x => {
			var item = new PO_MODIFIED_PO_SPEED_UP_EDITTABLE();
			item.pO_ID = x.pO_ID;
    ...
    onChangePOModifiedDT(item: PO_MODIFIED_PO_SPEED_UP_EDITTABLE){
