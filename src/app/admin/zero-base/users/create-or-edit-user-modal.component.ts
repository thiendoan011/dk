...
export class CreateOrEditUserModalComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    onUsernameFocusout(evt){
        if(!this.isLdapLoginMethod && !this.isAdfsLoginMethod){
            return;
        }
        let enable = true;
        if(!this.adUserValid || evt.target.value != this.adUserValid){
            enable = false;
        }
        $('#buttonSaveUser').prop('disabled', !enable);
    }
    ...
    getUserInfo() {
        let getUserCallback = (response) => {
            switch (response.result) {
                case LdapResultStatusConsts.Success:
                    ...
                    this.adUserValid = this.user.userName;
                    this.userNameValid = 'valid';
                    this.removeMessage();
                    this.showSuccessMessage(this.l('LdapResultStatusConstsSuccess'))
                    $('#buttonSaveUser').prop('disabled', false);
                    this.updateView();
                    break;
        ...
        if(this.isLdapLoginMethod){
            this._tokenAuthServiceProxy.getUserInfomation(this.user.userName, undefined).subscribe(getUserCallback);
        }
        if(this.isAdfsLoginMethod){
            this._tokenAuthServiceProxy.getUserADFSInfomation(this.user.userName).subscribe(getUserCallback);
        }
    }
