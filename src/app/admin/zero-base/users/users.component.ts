import { Component, Injector, ViewChild, ViewEncapsulation, OnInit, ChangeDetectorRef, AfterContentChecked, ElementRef } from '@angular/core';
...
export class UsersComponent extends AppComponentBase implements OnInit, AfterContentChecked {
    ...
    lockUser(record): void {
        let input = new EntityDtoOfInt64();
        input.id = record.id;
        this._userServiceProxy.lockUser(input).subscribe(() => {
            this.reloadPage();
            this.showSuccessMessage(this.l('LockedTheUser', record.userName));
        });
    }
    ...
    exportToExcel() {
        ...
        let reportFilter:any = { ...this.filterInput};

        reportFilter.maxResultCount = -1;
        reportFilter.top = 0;
        reportFilter.TLNANME = this.filterInput.filter;
        reportFilter.role = this.filterInput.role;
        reportFilter.permission = this.filterInput.permission;
        //reportFilter.independentUnit = this.
