...
import { AsposeServiceProxy, ReportInfo, TT_LEAVE_ENTITY, SupplierServiceProxy, LeaveServiceProxy, TL_USER_ENTITY, TlUserServiceProxy, LeaveNSServiceProxy } from '@shared/service-proxies/service-proxies';
...
import { TTSuggestModalComponent } from '@app/admin/core/modal/module-time-tracking/tt-suggest-modal/tt-suggest-modal.component';

@Component({
    templateUrl: './leave-ns-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class TTLeaveNSEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionEdit<TT_LEAVE_ENTITY> {
//#region constructor
    constructor(
        injector: Injector,
    ...
    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                this.appToolbar.setRole('TTLeaveNS', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('TTLeaveNS', false, false, false, false, false, false, true, false);
...
//#region CRUD    
    goBack() {
        this.navigatePassParam('/app/admin/leave-ns', null, { filterInput: JSON.stringify(this.filterInput) });
    }
    ...
    getDataPages() {
        this.leaveService.tT_LEAVE_ById(this.inputModel.leavE_ID).subscribe(response => {
            this.leaveService.tT_LEAVE_Permission(this.inputModel.leavE_ID, this.appSession.user.userName).subscribe(res_per => {
                ...
                this.setViewToolBar(res_per['IS_ENABLE_NS_SAVE'], res_per['IS_ENABLE_NS_APP'], res_per['IS_ENABLE_NS_REJECT']);

                // lịch sử xử lý
                this.history_modal.getDetail();
                // lịch sử từ chối
                this.reject_modal.getDetail();
    ...
    onUpdate(): void {
        ...
        this.saving = true;
        this.leaveNSService
        .tT_LEAVE_NS_Upd(this.inputModel)
        ...
        this.message.confirm(
            ...
            (isConfirmed) => {
                if (isConfirmed) {
                    this.saving = true;
                    this.leaveNSService
                    .tT_LEAVE_NS_App(this.inputModel.leavE_ID, this.appSession.user.userName)
                    .pipe(finalize(() => {this.saving = false}))
                    .subscribe((res) => {
                        if (res['Result'] != '0') {
                            this.showErrorMessage(res['ErrorDesc']);
...
//#region "EditTable"
    ...
    // đề xuất từ chối
    @ViewChild('sug_reject_modal') sug_reject_modal: TTSuggestModalComponent;
