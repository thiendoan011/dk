...
import { PUR_ORDER_ENTITY, PUR_RECEIPT_EDITTABLE, PUR_R_ENTITY, PUR_PO_ENTITY, PUR_GROUP_PRODUCT_ENTITY, PUR_PRODUCT_ENTITY, PUR_PRODUCT_GROUP_DETAIL_ENTITY, PUR_PRODUCT_DETAIL_ENTITY, PUR_MATERIAL_ENTITY, PurOrderServiceProxy, PUR_RECEIPT_MULTI_DTO } from "@shared/service-proxies/service-proxies";
import { finalize } from "rxjs/operators";

@Component({
    selector: 'pur-inventory-receipt-multi-edittable',
	templateUrl: './pur-inventory-receipt-multi-edittable.component.html'
})

export class PURInventoryReceiptMultiEdittableComponent extends ChangeDetectionComponent implements AfterViewInit {
//#region "Constructor"
    constructor(
        injector: Injector,
...
//#endregion "Constructor"    
    ...
    onUpdMulti(){
        abp.ui.setBusy();
        // set data
        let multiInventoryReceiptInput: PUR_RECEIPT_MULTI_DTO = new PUR_RECEIPT_MULTI_DTO();
        multiInventoryReceiptInput.puR_REQUISITION_ID = this.filterInput.puR_REQUISITION_ID;
        multiInventoryReceiptInput.materiaL_ID = this.filterInput.materiaL_ID;
        multiInventoryReceiptInput.updatE_DT = this.filterInput.updatE_DT;
        multiInventoryReceiptInput.puR_MULTI_Receipts = this.editTable.allData;
        
        this.purOrderService
        .pUR_RECEIPT_INVENTORY_MULTI_Ins(multiInventoryReceiptInput)
    ...
    totalInventoryReceived: number = 0;
    onChangeQuantity(){
        this.totalInventoryReceived = this.editTable.getSum('quantitY_INVENTORY_RECEIVED');
        this.updateView();
    }
