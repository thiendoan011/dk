...
import { VHE_DRIVER_ENTITY, TL_USER_ENTITY, TlUserServiceProxy, VHEDriverServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    templateUrl: './vhe-driver-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class VHEDriverEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionEdit<VHE_DRIVER_ENTITY> {
//#region constructor
    constructor(
        injector: Injector,
        ...
        this.inputModel.vhE_DRIVER_ID = this.getRouteParam('id');
        this.initDefaultFilter();
    }

    EditPageState = EditPageState;
    editPageState: EditPageState;
    inputModel: VHE_DRIVER_ENTITY = new VHE_DRIVER_ENTITY();
    filterInput: VHE_DRIVER_ENTITY;

    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                this.appToolbar.setRole('VHEDriver', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('VHEDriver', false, false, false, false, false, false, true, false);
...
//#region CRUD    
    goBack() {
        this.navigatePassParam('/app/admin/vhe-driver', null, { filterInput: JSON.stringify(this.filterInput) });
    }

    getDataPages() {
        this.driverService.vHE_DRIVER_ById(this.inputModel.vhE_DRIVER_ID).subscribe(response => {
            // set data
    ...
    saveInput() {
        ...
        if(!this.inputModel.vhE_DRIVER_ID) {
            this.onAdd();
        } else {
            this.onUpdate();
        }
    ...
    onAdd(): void {
        this.saving = true;
        this.driverService
        .vHE_DRIVER_Ins(this.inputModel)
        ...
        .subscribe(res => {
            if(res['Result'] != '0'){
                ...
                this.inputModel.vhE_DRIVER_ID = res['ID'];
    ...
    onUpdate(): void {
        this.saving = true;
        this.driverService
        .vHE_DRIVER_Upd(this.inputModel)
        .pipe(finalize(() => {this.saving = false}))
        .subscribe((res) => {
            if (res['Result'] != '0') {
                this.showErrorMessage(res['ErrorDesc']);
    ...
    onApprove(item: VHE_DRIVER_ENTITY): void{
    }
...
//#region "EditTable"
    ...
    // Danh sách vật tư
    //@ViewChild('purOrderDetailEdittable') purOrderDetailEdittable: VHEDriverDetailEdittableComponent;
    // lịch sử xử lý
