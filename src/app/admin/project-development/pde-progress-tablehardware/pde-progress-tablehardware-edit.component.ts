...
import { PDEProgressTablehardwareAttachFileComponent } from './pde-progress-tablehardware-attach-file.component';
...
@Component({
    templateUrl: './pde-progress-tablehardware-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class PDEProgressTablehardwareEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionEdit<PO_GROUP_PRODUCT_ENTITY> {
//#region "Constructor"
    constructor(
        injector: Injector,
...
//#endregion "Constructor"
    ...
    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                this.appToolbar.setRole('PDEProgressTablehardware', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('PDEProgressTablehardware', false, false, false, false, false, false, true, false);
    ...
    goBack() {
        this.navigatePassParam('/app/admin/pde-progress-tablehardware', null, { filterInput: JSON.stringify(this.filterInput) });
    }

    getDataPages() {
        this.pdeGroupProductService.pDE_GROUP_PRODUCT_PROGRESS_TABLEHARDWARE_Byid(this.inputModel.grouP_PRODUCT_ID).subscribe(res => {
            this.pdeGroupProductService.cHECK_STATUS_PDE_GROUP_PRODUCT_ById(this.inputModel.grouP_PRODUCT_ID, 'GROUP_PRODUCT_PROGRESS_TABLEHARDWARE', this.appSession.user.userName).subscribe(res_status => {
                // set data
                if (!res) this.goBack()
                this.inputModel = res;
    ...
    onUpdate(): void {
        ...
        this.pdeGroupProductService
			.pDE_GROUP_PRODUCT_PROGRESS_TABLEHARDWARE_Upd(this.inputModel)
			.pipe(
				finalize(() => {
					abp.ui.clearBusy();
				})
...
//#region "EditTable"
    getDataEditTables(){
        ...
        this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs = this.pdeProgressTablehardwareAttachFileEdittable.editTable.allData;
    ...
    setDataEditTables(){
        ...
        if (this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs && this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs.length > 0) {
            this.pdeProgressTablehardwareAttachFileEdittable.editTable.setList(this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs);
            this.pdeProgressTablehardwareAttachFileEdittable.refreshTable();
        }
        // Thông tin yêu cầu từ khách
    ...
    // Thông tin báo giá từ bên bộ phận dự án mẫu gửi
    @ViewChild('pdeProgressTablehardwareAttachFileEdittable') pdeProgressTablehardwareAttachFileEdittable: PDEProgressTablehardwareAttachFileComponent;
    // lịch sử xử lý
...
//#endregion "EditTable"
    ...
    get showButtonConfirmProgressTablehardware(): boolean {
        return this.statusModel.grouP_PRODUCT_CONFIRM_PROGRESS_TABLEHARDWARE == '0';
    }
    onConfirmProgressTablehardware(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_PROGRESS_TABLEHARDWARE_Confirm(this.inputModel.grouP_PRODUCT_ID ,this.appSession.user.userName)
    ...
    get showCompleteButton(): boolean {
        return this.statusModel.grouP_PRODUCT_REQUEST_TABLEHARDWARE_COMPLETE == '0';
    }
    onComplete(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_REQUEST_TABLEHARDWARE_Complete(this.inputModel.grouP_PRODUCT_ID, this.appSession.user.userName)
        .pipe( finalize(() => { abp.ui.clearBusy(); }) )
        .subscribe((res) => {
            if (res['Result'] != '0') {
                this.showErrorMessage(res['ErrorDesc']);
