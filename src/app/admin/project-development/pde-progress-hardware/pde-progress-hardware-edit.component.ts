...
import { PDEProgressHardwareAttachFileComponent } from './pde-progress-hardware-attach-file.component';
...
@Component({
    templateUrl: './pde-progress-hardware-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class PDEProgressHardwareEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionEdit<PO_GROUP_PRODUCT_ENTITY> {
//#region "Constructor"
    constructor(
        injector: Injector,
...
//#endregion "Constructor"
    ...
    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                this.appToolbar.setRole('PDEProgressHardware', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('PDEProgressHardware', false, false, false, false, false, false, true, false);
    ...
    goBack() {
        this.navigatePassParam('/app/admin/pde-progress-hardware', null, { filterInput: JSON.stringify(this.filterInput) });
    }

    getDataPages() {
        this.pdeGroupProductService.pDE_GROUP_PRODUCT_PROGRESS_HARDWARE_Byid(this.inputModel.grouP_PRODUCT_ID).subscribe(res => {
            this.pdeGroupProductService.cHECK_STATUS_PDE_GROUP_PRODUCT_ById(this.inputModel.grouP_PRODUCT_ID, 'GROUP_PRODUCT_PROGRESS_HARDWARE', this.appSession.user.userName).subscribe(res_status => {
                // set data
                if (!res) this.goBack()
                this.inputModel = res;
    ...
    onUpdate(): void {
        ...
        this.pdeGroupProductService
			.pDE_GROUP_PRODUCT_PROGRESS_HARDWARE_Upd(this.inputModel)
			.pipe(
				finalize(() => {
					abp.ui.clearBusy();
				})
...
//#region "EditTable"
    getDataEditTables(){
        ...
        this.inputModel.pdE_REQ_HARDWARE_ATTACH_FILEs = this.pdeProgressHardwareAttachFileEdittable.editTable.allData;
    ...
    setDataEditTables(){
        ...
        if (this.inputModel.pdE_REQ_HARDWARE_ATTACH_FILEs && this.inputModel.pdE_REQ_HARDWARE_ATTACH_FILEs.length > 0) {
            this.pdeProgressHardwareAttachFileEdittable.editTable.setList(this.inputModel.pdE_REQ_HARDWARE_ATTACH_FILEs);
            this.pdeProgressHardwareAttachFileEdittable.refreshTable();
        }
        // Thông tin yêu cầu từ khách
    ...
    // Thông tin báo giá từ bên bộ phận dự án mẫu gửi
    @ViewChild('pdeProgressHardwareAttachFileEdittable') pdeProgressHardwareAttachFileEdittable: PDEProgressHardwareAttachFileComponent;
    // lịch sử xử lý
...
//#endregion "EditTable"
    ...
    get showButtonConfirmProgressHardware(): boolean {
        return this.statusModel.grouP_PRODUCT_CONFIRM_PROGRESS_HARDWARE == '0';
    }
    onConfirmProgressHardware(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_PROGRESS_HARDWARE_Confirm(this.inputModel.grouP_PRODUCT_ID ,this.appSession.user.userName)
    ...
    get showCompleteButton(): boolean {
        return this.statusModel.grouP_PRODUCT_REQUEST_HARDWARE_COMPLETE == '0';
    }
    onComplete(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_REQUEST_HARDWARE_Complete(this.inputModel.grouP_PRODUCT_ID, this.appSession.user.userName)
        .pipe( finalize(() => { abp.ui.clearBusy(); }) )
        .subscribe((res) => {
            if (res['Result'] != '0') {
                this.showErrorMessage(res['ErrorDesc']);
