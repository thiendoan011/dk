...
import { PDEReqPriceAttachFileComponent } from './pde-req-price-attach-file.component';
...
@Component({
    templateUrl: './pde-req-price-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class PDEReqPriceEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionEdit<PO_GROUP_PRODUCT_ENTITY> {
//#region "Constructor"
    constructor(
        injector: Injector,
...
//#endregion "Constructor"
    ...
    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                this.appToolbar.setRole('PDEReqPrice', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('PDEReqPrice', false, false, false, false, false, false, true, false);
    ...
    goBack() {
        this.navigatePassParam('/app/admin/pde-req-price', null, { filterInput: JSON.stringify(this.filterInput) });
    }

    getDataPages() {
        this.pdeGroupProductService.pDE_GROUP_PRODUCT_PROGRESS_PRICE_Byid(this.inputModel.grouP_PRODUCT_ID).subscribe(res => {
            this.pdeGroupProductService.cHECK_STATUS_PDE_GROUP_PRODUCT_ById(this.inputModel.grouP_PRODUCT_ID, 'GROUP_PRODUCT_REQ_PRICE', this.appSession.user.userName).subscribe(res_status => {
                // set data
                if (!res) this.goBack()
                this.inputModel = res;
...
//#region "EditTable"
    getDataEditTables(){
        ...
        this.inputModel.pdE_REQ_PRICE_ATTACH_FILEs = this.pdeReqPriceAttachFileEdittable.editTable.allData;
    ...
    setDataEditTables(){
        ...
        if (this.inputModel.pdE_REQ_PRICE_ATTACH_FILEs && this.inputModel.pdE_REQ_PRICE_ATTACH_FILEs.length > 0) {
            this.pdeReqPriceAttachFileEdittable.editTable.setList(this.inputModel.pdE_REQ_PRICE_ATTACH_FILEs);
            this.pdeReqPriceAttachFileEdittable.refreshTable();
        }
        // Thông tin yêu cầu từ khách
    ...
    // Thông tin báo giá từ bên bộ phận dự án mẫu gửi
    @ViewChild('pdeReqPriceAttachFileEdittable') pdeReqPriceAttachFileEdittable: PDEReqPriceAttachFileComponent;
    // Thông tin yêu cầu từ khách
...
//#region Button Flow
    get showAcceptButton(): boolean {
        return this.inputModel.reQ_PRICE_REQ_STATUS == 'U';
    }
    onAccept(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_REQUEST_PRICE_Accept(this.inputModel.grouP_PRODUCT_ID, this.appSession.user.userName)
    ...
    onShowNotAccept(){
        ...
        this.cmRejectModal.rejectLogInput.stage = 'PDE_REQ_PRICE_SEND';
    ...
    initCombobox(): void {
        ...
        filter.tlsubbrid = this.inputModel.reQ_PRICE_REQ_BRANCH_ID;
        filter.deP_ID = this.inputModel.reQ_PRICE_REQ_DEP_ID;
        filter.level = 'ALL';
    ...
    get showSendProgressPrice(): boolean {
        return this.statusModel.grouP_PRODUCT_SEND_PROGRESS_PRICE == '0';
    }
    onSendProgressPrice(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_PROGRESS_PRICE_Send(this.inputModel.grouP_PRODUCT_ID ,this.appSession.user.userName, this.inputModel.reQ_PRICE_PROGRESS_EXACT_MAKER_ID)
        .pipe( finalize(() => { abp.ui.clearBusy(); }) )
        .subscribe((res) => {
            if (res['Result'] != '0') {
                this.showErrorMessage(res['ErrorDesc']);
    ...
    get showExportPYC(): boolean {
        return this.inputModel.reQ_PRICE_REQ_STATUS == 'A';
    }
    onExportPYC(){
        abp.ui.setBusy();
		let reportInfo = new ReportInfo();
		reportInfo.typeExport = ReportTypeConsts.Excel;
        reportInfo.pathName = '/PDE/PDE_GROUP_PRODUCT_REQUEST_PRICE_EXPORT_PYC.xlsx';
		reportInfo.storeName = 'PDE_GROUP_PRODUCT_REQUEST_PRICE_EXPORT_PYC';
