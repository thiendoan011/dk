...
import { PDETablehardwareEditTableComponent } from './pde-tablehardware-edittable.component';
...
@Component({
    templateUrl: './pde-req-tablehardware-edit.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})
export class PDEReqTablehardwareEditComponent extends DefaultComponentBase implements OnInit, AfterViewInit, IUiActionEdit<PO_GROUP_PRODUCT_ENTITY> {
//#region "Constructor"
    constructor(
        injector: Injector,
...
//#endregion "Constructor"
    ...
    ngOnInit(): void {
        switch (this.editPageState) {
            case EditPageState.add:
                this.appToolbar.setRole('PDEReqtablehardware', false, false, true, false, false, false, false, false);
            ...
            case EditPageState.viewDetail:
                this.appToolbar.setRole('PDEReqtablehardware', false, false, false, false, false, false, true, false);
    ...
    goBack() {
        this.navigatePassParam('/app/admin/pde-req-tablehardware', null, { filterInput: JSON.stringify(this.filterInput) });
    }

    getDataPages() {
        this.pdeGroupProductService.pDE_GROUP_PRODUCT_REQUEST_TABLEHARDWARE_Byid(this.inputModel.grouP_PRODUCT_ID).subscribe(res => {
            this.pdeGroupProductService.cHECK_STATUS_PDE_GROUP_PRODUCT_ById(this.inputModel.grouP_PRODUCT_ID, 'GROUP_PRODUCT_REQ_TABLEHARDWARE', this.appSession.user.userName).subscribe(res_status => {
                // set data
                if (!res) this.goBack()
                this.inputModel = res;
...
//#region "EditTable"
    getDataEditTables(){
        ...
        this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs = this.pdeReqtablehardwareAttachFileEdittable.editTable.allData;
        ...
        // Danh sách vật tư
        this.inputModel.pdE_GROUP_PRODUCT_TABLEHARDWAREs = this.pdeTablehardwareEdittable.editTable.allData;
    }
    setDataEditTables(){
        // Thông tin báo giá từ bên bộ phận dự án mẫu gửi
        if (this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs && this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs.length > 0) {
            this.pdeReqtablehardwareAttachFileEdittable.editTable.setList(this.inputModel.pdE_REQ_TABLEHARDWARE_ATTACH_FILEs);
            this.pdeReqtablehardwareAttachFileEdittable.refreshTable();
        }
        // Thông tin yêu cầu từ khách
        ...
        // }
        // Danh sách vật tư
        if (this.inputModel.pdE_GROUP_PRODUCT_TABLEHARDWAREs.length > 0) {
            this.pdeTablehardwareEdittable.editTable.setList(this.inputModel.pdE_GROUP_PRODUCT_TABLEHARDWAREs);
        }
    }
    // Thông tin báo giá từ bên bộ phận dự án mẫu gửi
    @ViewChild('pdeReqtablehardwareAttachFileEdittable') pdeReqtablehardwareAttachFileEdittable: PDEProgressTablehardwareAttachFileComponent;
    // Thông tin yêu cầu từ khách
    ...
    // Danh sách vật tư
    @ViewChild('pdeTablehardwareEdittable') pdeTablehardwareEdittable: PDETablehardwareEditTableComponent;

//#endregion "EditTable"

//#region Button Flow
    get showAcceptButton(): boolean {
        return this.inputModel.reQ_TABLEHARDWARE_REQ_STATUS == 'U';
    }
    onAccept(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_REQUEST_TABLEHARDWARE_Accept(this.inputModel.grouP_PRODUCT_ID, this.appSession.user.userName)
    ...
    onShowNotAccept(){
        ...
        this.cmRejectModal.rejectLogInput.stage = 'PDE_REQ_TABLEHARDWARE_SEND';
    ...
    initCombobox(): void {
        ...
        filter.tlsubbrid = this.inputModel.reQ_TABLEHARDWARE_REQ_BRANCH_ID;
        filter.deP_ID = this.inputModel.reQ_TABLEHARDWARE_REQ_DEP_ID;
        filter.level = 'ALL';
    ...
    get showSendProgressTablehardware(): boolean {
        return this.statusModel.grouP_PRODUCT_SEND_PROGRESS_TABLEHARDWARE == '0';
    }
    onSendProgresstablehardware(){
        abp.ui.setBusy();
        this.pdeGroupProductService
        .pDE_GROUP_PRODUCT_PROGRESS_TABLEHARDWARE_Send(this.inputModel.grouP_PRODUCT_ID ,this.appSession.user.userName, this.inputModel.reQ_TABLEHARDWARE_PROGRESS_EXACT_MAKER_ID)
        .pipe( finalize(() => { abp.ui.clearBusy(); }) )
        .subscribe((res) => {
            if (res['Result'] != '0') {
                this.showErrorMessage(res['ErrorDesc']);
    ...
    get showExportPYC(): boolean {
        return this.inputModel.reQ_TABLEHARDWARE_REQ_STATUS == 'A';
    }
    onExportPYC(){
        abp.ui.setBusy();
		let reportInfo = new ReportInfo();
		reportInfo.typeExport = ReportTypeConsts.Excel;
        reportInfo.pathName = '/PDE/PDE_GROUP_PRODUCT_REQUEST_TABLEHARDWARE_EXPORT_PYC.xlsx';
		reportInfo.storeName = 'PDE_GROUP_PRODUCT_REQUEST_TABLEHARDWARE_EXPORT_PYC';
