...
@Component({
    ...
    templateUrl: "./file-system-uploader.component.html",
    encapsulation: ViewEncapsulation.None,
    providers: [createCustomInputControlValueAccessor(FileSystemUploaderComponent)]
})

export class FileSystemUploaderComponent extends ControlComponent implements OnInit, OnDestroy {
    ...
    idProgress: string;
    linkServer: string = '/RoxyFileman/UPLOAD_SYSTEM';
    ...
    async fileUpload(i) {
        ...
        let headers = { 'Authorization': 'Bearer ' + tokenAuth, 'Accept': 'application/json, text/plain, */*', 'Access-Encoding': 'gzip, deflate', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, POST, PATCH, PUT, DELETE, OPTIONS', 'Access-Control-Allow-Headers': 'Origin,X-Requested-With, Content-Type, X-Auth-Token' };
        ...
        try {
            ...
            this.httpClient.request(req).subscribe(event => {
                ...
                if (event instanceof HttpResponse) {
                    ...
                    this.hideLoading();
                    this.updateView();
                    var result = { Result: event.body["result"].result, Message: event.body["result"].message };
                    if (result.Result == '-1') {
                        // this.showErrorMessage(result.Message);
                        this.showErrorMessage(this.l('ImportFileError'));
                        return;
                    }
                    this.showSuccessMessage(this.l('ImportFileSuccess'));
                    // sub.unsubscribe();
                }
            })
