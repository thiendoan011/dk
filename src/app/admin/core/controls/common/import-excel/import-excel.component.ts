import { Component, Input, ViewEncapsulation, Output, Injector, Optional, Inject, ViewChild, SimpleChanges } from "@angular/core";
...
export class ImportExcelComponent extends ComponentBase {
    ...
    *iiCreateIteratorWithStartPosition(keys: string[], obj: Object, startPosition: string): IterableIterator<Object> {
    ...
    clearInputFile(): void {
        if ((<HTMLInputElement>document.getElementById(this.id)) == null)
            return;
        (<HTMLInputElement>document.getElementById(this.id)).value = '';
    }
