    ...
    initInputTooltip() {
        ...
        $(this.table.nativeElement).find('tbody>tr>td').each(function (index, value) {
            if ($(value).find(':input').length == 0) {
                ...
                var textContent = '';
                textContent = scope.encodeHTML(($(value).find(':input:first')[0] as HTMLInputElement).value);
                var colWidth = $(value).width();
                if (colWidth / textContent.length < 10) {
                    $(value).tooltip({
                        items: "td, input",
            ...
            trs.each(function () {
                ...
                for (var inh = 0; inh < columns.length; inh++) {
                    ...
                    if (spans.length) {
                        ...
                        text = (th as HTMLElement).innerText;
                    }
                    ...
                    if (mobileSpan.length) {
                        (mobileSpan[0] as HTMLElement).textContent = text;
                    }
                }
            })
        }
