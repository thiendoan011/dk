import { ViewEncapsulation, Component, Input, AfterViewInit, ViewChild, ElementRef, AfterContentInit, Output, EventEmitter, AfterViewChecked, AfterContentChecked, DoCheck, ChangeDetectorRef, Renderer2, Injector, OnInit } from "@angular/core";
...
export class CoreTableComponent<T> implements OnInit, AfterViewInit, AfterContentChecked {
    ...
    resizableColumns = abp.setting.getBoolean("gAMSProCommon.ResizableColumns");
            ...
            for (let tr of Array.from(trs)) {
                ...
                for (var inh = 0; inh < columns.length; inh++) {
                    ...
                    if (mobileSpan.length) {
                        (mobileSpan[0] as HTMLElement).innerText = text;
                    }
                }
            }
        }
