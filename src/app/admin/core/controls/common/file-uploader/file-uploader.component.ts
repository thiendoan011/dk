import { Component, Input, OnInit, Injector, ViewEncapsulation, Output, Inject, Optional, ViewChild, ElementRef, inject, OnDestroy, HostListener, EventEmitter } from "@angular/core";
...
export class FileUploaderComponent extends ControlComponent implements OnInit, OnDestroy {
    ...
    @Output() attach_infor_out: EventEmitter<any> = new EventEmitter<any>();
    ...
    viewFile(item: CM_ATTACH_FILE_ENTITY){
        if(item.filE_TYPE == '.pdf'){
            var url = this.remoteServiceBaseUrl + '/' + encodeURIComponent(item.patH_NEW + '/' + item.filE_NAME_NEW);
            window.open(url);
        }
    }
    ...
    uploadComplete(e, i) {
        ...
        this.attach_infor_out.emit(this.ngModel);
        this.hideLoading();
        this.updateView();
    }
    ...
    async fileUpload(i) {
        ...
        for (var inn = 0; inn < el.files.length; inn++) {
            if(!this.accept.includes(el.files[inn].name.split('.').pop())){
            ...
            if(el.files[inn].size > (MaxSize *1024*1024)){
                this.showErrorMessage(`Dung lượng file vượt quá quy định! Hệ thống chỉ cho phép đính kèm file có dung lượng dưới ${MaxSize} MB`);
                this.isShowError=true;
          
                this.errors="MaxFileLengthInvalid";
                this.updateView();
                return;
            }
        }
