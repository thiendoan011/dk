...
import { HISTORY_DTO, VHEHistoryServiceProxy } from '@shared/service-proxies/service-proxies';
import * as moment from 'moment';
import { finalize } from 'rxjs/operators';

@Component({
    selector: 'vhe-reject-modal',
    templateUrl: './vhe-reject-modal.component.html',
})

export class VHERejectModalComponent extends DefaultComponentBase implements OnInit {
    constructor(injector: Injector, 
        private historyService : VHEHistoryServiceProxy,) {
        super(injector);
    }
...
// Lưới nhập lý do trả về
    onReject(type_reject: string) {
        ...
        abp.ui.setBusy();
        this.historyService.vHE_HISTORY_Ins(this.rejectLogInput)
...
// Lưới lịch sử trả về
    ...
    getReject(): void {
        if(this.showRejectLog){
            this.historyService.vHE_HISTORY_Search(this.id, this.page, this.stage, this.process, this.appSession.user.userName).subscribe(response => {
