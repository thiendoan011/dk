...
export class ProductDetailListComponent extends ListComponentBase<PRODUCT_DETAIL_ENTITY> implements IUiActionList<PRODUCT_DETAIL_ENTITY>, OnInit, AfterViewInit {
...
//#region search and navigation
    ...
    onAdd(): void {
        this.navigatePassParam('/app/admin/product-detail-add', null, { filterInput: JSON.stringify(this.filterInputSearch) });
    }

    onUpdate(item: PRODUCT_DETAIL_ENTITY): void {
        this.navigatePassParam('/app/admin/product-detail-edit', { id: item.producT_DETAIL_ID }, { filterInput: JSON.stringify(this.filterInputSearch) });
    }

    onViewDetail(item: PRODUCT_DETAIL_ENTITY): void {
        this.navigatePassParam('/app/admin/product-detail-view', { id: item.producT_DETAIL_ID }, { filterInput: JSON.stringify(this.filterInputSearch) });
    }

    onDelete(item: PRODUCT_DETAIL_ENTITY): void {
        this.message.confirm(
            this.l('DeleteWarningMessage', item.producT_DETAIL_CODE),
            this.l('AreYouSure'),
            (isConfirmed) => {
                if (isConfirmed) {
                    this.saving = true;
                    this.productDetailService.pRODUCT_DETAIL_Del(item.producT_DETAIL_ID, this.appSession.user.userName)
