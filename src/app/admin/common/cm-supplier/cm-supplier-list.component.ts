...
@Component({
    templateUrl: './cm-supplier-list.component.html',
    encapsulation: ViewEncapsulation.None,
    animations: [appModuleAnimation()]
})

export class CMSupplierListComponent extends ListComponentBase<CM_SUPPLIER_ENTITY> implements IUiActionList<CM_SUPPLIER_ENTITY>, OnInit, AfterViewInit {
...
//#region constructor
    ...
    ngOnInit(): void {
        ...
        // set role toolbar
        this.appToolbar.setRole('CMSupplier', true, true, false, true, true, true, false, true);
...
//#region search and navigation
    ...
    onAdd(): void {
        this.navigatePassParam('/app/admin/cm-supplier-add', null, { filterInput: JSON.stringify(this.filterInputSearch) });
    }

    onUpdate(item: CM_SUPPLIER_ENTITY): void {
        this.navigatePassParam('/app/admin/cm-supplier-edit', { id: item.suP_ID }, { filterInput: JSON.stringify(this.filterInputSearch) });
    }

    onViewDetail(item: CM_SUPPLIER_ENTITY): void {
        this.navigatePassParam('/app/admin/cm-supplier-view', { id: item.suP_ID }, { filterInput: JSON.stringify(this.filterInputSearch) });
    }

    onDelete(item: CM_SUPPLIER_ENTITY): void {
        this.message.confirm(
            this.l('DeleteWarningMessage', item.suP_CODE),
            this.l('AreYouSure'),
            (isConfirmed) => {
                if (isConfirmed) {
                    this.saving = true;
