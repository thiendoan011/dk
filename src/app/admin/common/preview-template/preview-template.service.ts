...
import styleBundle from '../../../../assets/metronic/assets/demo/default/base/style.bundle.css'
...
export class PreviewTemplateService extends EvalFunctionComponent {
    ...
    private ProcessGroupByMultiLevelRow(reportTable: ReportTable, groupProperty: string[], table: string, level: number = 0, tableStart: string, tableEnd: string, result: string = ''): string {
        ...
        for (var item of tableGroupBy) {
            ...
            addDataToTemplate = this.escapeHtml(addDataToTemplate).split(this.escapeHtml($('tr:first', jquery).html())).join(evalForStartGroup);
            addDataToTemplate = this.escapeHtml(addDataToTemplate).split(this.escapeHtml($('tr:last', jquery).html())).join(evalForEndGroup);
                ...
                for (var i = 1; i < storeData.length; i++) {
                    ...
                    if (reportContent.match(regexTableGroup) && reportContent.search(tableEnd) > -1) {
                        while (reportContent.match(regexTableGroup) && reportContent.search(tableEnd) > -1) {
                            ...
                            var squareRegex = /\[([^)]+)\]/g;
                            var groupProperty = squareRegex.exec(groupStart)[1];
    ...
    private QRCodeGenerator(template: string): string {
        ...
        while (template.match(regexEval)) {
            ...
            var qrContent = $(document.createElement('div')).html(qrString.match(/\{(.*?)\}/)[1].trim()).text();
    ...
    print(data: string) {
        var css = printPagecss;
        if (data && data.length > 0 && data.indexOf('parent-page pck')) {
            css += styleBundle;
        }
        var printDivCSS = "<style> " + css + "</style>";

        data = printDivCSS + data;
        // data = `
        // <style>
        //     body {
        //         margin: 0 !important;
        ...
        newWindow.document.write('<html><head> <style>body {counter-reset: page;}</style> <meta name="viewport" content="width=device-width"></head><body onload="window.print()">' + data + '</body></html>');
    ...
    printReportTemplate(reportTemplateCode: string, params: ReportParameter[], values: ReportParameter[]) {
        ...
        this._reportTemplateService.cM_REPORT_TEMPLATE_ByCode(reportTemplateCode).pipe(finalize(()=>{
            abp.ui.clearBusy();
        })).subscribe(response => {
            abp.ui.setBusy(null, this.l('SavingWithThreeDot'));
